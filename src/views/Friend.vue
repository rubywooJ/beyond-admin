                  ...
                  <el-tab-pane label="技术区" name="first" v-if="frilist.team == '技术区'">
                    <table>
                      <tr v-for="(teams, j) in frilist.links" :key="j">
                        ...
                        <td class="fridslink">
                          <a v-bind:href="origin" target="_blank">
                            {{ teams.url
                            }}
                          </a>
                        </td>
                        <td class="fridsoper">
                          <span
                            v-on:click="getfrid(teams.id, teams.name, teams.url, teams.img, teams.team, teams.description), toggle2()"
                          >{{ operate }}</span>
                          |
                          <span
                            v-on:click="getfrid(teams.id), deleFrids()"
                          >{{ dele }}</span>
                        </td>
                      </tr>
                    </table>
...
export default {
  ...
  methods: {
    ...
    deleFrids() {
      linkApi.delete(this.friendId).then(response => {
        // console.log(response.data.data);
        this.getFriends();
      });
    }
